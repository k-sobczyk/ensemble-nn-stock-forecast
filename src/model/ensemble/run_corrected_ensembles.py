    ...
    for combo_name, combo_type in all_combinations:
        ...
        try:
            ...
            result = runner.run_combination(
                combo_name, models_config, ensemble_methods=['voting', 'blending', 'stacking']
            )
            ...
            for method in ['voting', 'blending', 'stacking']:
                ...
                if method in result and 'error' not in result[method]:
                    print(
                        f'  ✅ {combo_name} {method}: RMSE {result[method]["metrics"]["rmse"]:.4f}, R² {result[method]["metrics"]["r2"]:.4f}'
                    )
                    successful_tests += 1
                else:
