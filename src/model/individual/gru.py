...
from src.model.individual.config import (
    ...
    GRU_HIDDEN_SIZE,
    GRU_LEARNING_RATE,
    GRU_LR_SCHEDULER_FACTOR,
    GRU_LR_SCHEDULER_PATIENCE,
    GRU_MAX_GRAD_NORM,
    GRU_NUM_LAYERS,
    GRU_WEIGHT_DECAY,
)
...
class GRU(nn.Module):
    def __init__(self, input_size, hidden_size=GRU_HIDDEN_SIZE, num_layers=GRU_NUM_LAYERS, dropout=GRU_DROPOUT):
        super().__init__()
...
def train_gru_model(
    ...
    lr_scheduler_patience=GRU_LR_SCHEDULER_PATIENCE,
    lr_scheduler_factor=GRU_LR_SCHEDULER_FACTOR,
    lr_scheduler_min_lr=1e-6,
    max_grad_norm=GRU_MAX_GRAD_NORM,
...
def main(
    ...
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model, train_losses, test_losses = train_gru_model(
        X_train,
        y_train,
        X_test,
        y_test,
    ...
    # Save loss plot
    os.makedirs('src/model/individual/results/gru_results', exist_ok=True)
    from src.model.individual.run_all_models import plot_and_save_loss, save_individual_model_results

    plot_and_save_loss(train_losses, test_losses, 'src/model/individual/results/gru_results/gru_loss.png', 'GRU')
    ...
    save_individual_model_results('GRU', results, train_losses, test_losses, 'src/model/individual/results')
